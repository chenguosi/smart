...
public class MenuController {
    ...
    @RequiresPermissions(value = {"菜单管理"})
    public String loadCheckMenuInfo(Integer parentId)
        throws Exception {
    ...
    public Map<String, Object> selectMenuById(Integer id) {
        ...
        try {
            if (id == null || id == 0) {
                ...
                if (tmenu == null) {
                    resultmap.put("state", "fail");
                    resultmap.put("msg", "无法找到该节点对象");
                    return resultmap;
                }
                else {
                    resultmap.put("state", "success");
                    resultmap.put("msg", "获取该节点对象成功");
        ...
        try {
            if (tmenu.getId() == null) {// 新建
                ...
                if (tmenu.getpId() == null || tmenu.getpId() == 0) {
                    ...
                    if (pmenu.getState() == 3) {
                        resultmap.put("state", "fail");
                        resultmap.put("msg", "3级菜单不可再添加子菜单");
                        return resultmap;
                    }
                    if ("-1".equalsIgnoreCase(String.valueOf(pmenu.getpId()))
                        ...
                        if (isNumeric(tmenu.getName())) {
                            resultmap.put("state", "fail");
                            resultmap.put("msg", "1级菜单的名字不可为纯数字");
                            return resultmap;
                        }
                ...
                List<Tmenu> menuList = authorService.selectMenuByName(tmenu.getName());
                if (menuList.size() > 0 && menuList.get(0).getId() != tmenu.getId()) {// 如果本次提交的名字在本次修改的节点之外已经存在
                    resultmap.put("state", "fail");
                    resultmap.put("msg", "当前菜单名已存在");
                    return resultmap;
                }
